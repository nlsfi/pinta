...
def _insert_raster_with_value(
    session: sqlmodel.Session,
    schema: str,
    table_name: str,
    value: float,
) -> None:
    """Insert one 256x256 raster tile at the default anchor filled with value."""
...
def _pixel_value(
    session: sqlmodel.Session, schema: str, table_name: str, x: int, y: int
) -> float:
    return session.exec(  # type: ignore[call-overload]
        sa.text(
            f'SELECT ST_Value(rast, 1, :x, :y) FROM "{schema}"."{table_name}"'
        ).bindparams(x=x, y=y)
    ).one()[0]


def test_merge_staging_tables_unions_tiles_existing_in_main_table(
    job_db: sqlmodel.Session,
):
    """Test staged tiles overlapping main table tiles are unioned, not duplicated."""
    table_name = "test_raster_merge_existing"
    schema = _SCHEMA
    _create_template_raster_table(job_db, schema, table_name)
    raster.initialize_raster_table(
        table_name=table_name, schema=schema, staging_tables=1, session=job_db
    )

    _insert_raster_with_value(job_db, schema, table_name, value=1)
    # Staged tile has data only in its upper-left pixel
    _insert_raster_with_value(job_db, schema, f"{table_name}_p0", value=-9999)
    job_db.exec(  # type: ignore[call-overload]
        sa.text(
            f'UPDATE "{schema}"."{table_name}_p0" '
            "SET rast = ST_SetValue(rast, 1, 1, 1, 2)"
        )
    )
    job_db.commit()
    ...
    assert _pixel_value(job_db, schema, table_name, 1, 1) == 2
    assert _pixel_value(job_db, schema, table_name, 2, 2) == 1
    _assert_staging_tables_does_not_exist(job_db, schema, table_name)


def test_merge_staging_tables_is_rerunnable(job_db: sqlmodel.Session):
    """Test merging the same staged tile twice does not violate uniqueness."""
    table_name = "test_raster_merge_rerun"
    schema = _SCHEMA
    _create_template_raster_table(job_db, schema, table_name)
    ...
    for _ in range(2):
        ...
        _insert_raster_with_value(job_db, schema, f"{table_name}_p0", value=1)
        job_db.commit()
        raster.merge_staging_tables(
            table_name=table_name, schema=schema, staging_tables=1, session=job_db
        )
    ...
    assert _row_count(job_db, schema, table_name) == 1
    assert _pixel_value(job_db, schema, table_name, 1, 1) == 1
