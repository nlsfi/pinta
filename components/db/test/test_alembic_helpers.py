...
from pinta_db_utils.alembic_helpers import render_item
...
def test_render_item_replaces_srid(
    ...
    mocker.patch(
        "pinta_db_utils.alembic_helpers.geoalchemy_alembic_helpers.render_item",
        return_value="geoalchemy2.types.Geometry(geometry_type='POINT', srid=3067)",
    )

    result = render_item("type", object(), autogen_context)
...
def test_render_item_adds_import(
    ...
    render_item("type", object(), autogen_context)
