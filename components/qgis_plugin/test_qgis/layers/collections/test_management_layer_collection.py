...
def test_add_to_project_adds_actions_to_production_area_layer(
    ...
    mock_add_register_action = mocker.patch.object(
        management_layer_collection,
...
def test_add_start_register_update_areas_action_delegates_to_layer_utils(
    mocker: MockerFixture,
):
    layer = mocker.MagicMock()
    mock_add_action = mocker.patch.object(
    ...
    management_layer_collection._add_start_register_update_areas_action(layer)
    ...
    assert (
        call.kwargs["description"]
        == "Start register update areas workflow for production area"
    )
    assert call.kwargs["short_title"] == "Register update areas"
    command = call.kwargs["command"]
    assert "from pinta_qgis_plugin.workflows import update_area" in command
