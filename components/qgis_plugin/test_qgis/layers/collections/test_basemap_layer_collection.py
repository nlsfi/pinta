...
from pinta_qgis_plugin.layers.collections import basemap_layer_collection


@pytest.fixture
def layer_collection() -> basemap_layer_collection.BasemapLayerCollection:
    return basemap_layer_collection.BasemapLayerCollection()
...
def test_add_to_project_with_valid_layer_adds_to_project(
    layer_collection: basemap_layer_collection.BasemapLayerCollection,
    mocker: MockerFixture,
    mock_qgs_project: MagicMock,
    production_area_layer: QgsVectorLayer,
):
    mocker.patch.object(
        basemap_layer_collection.raster_layer,
        "create_raster_layer",
        autospec=True,
        return_value=production_area_layer,
    )
    layer_collection.add_to_project()
    assert mock_qgs_project.addMapLayer.call_count == len(config.BASEMAP_LAYERS)
...
def test_remove_layers_removes_all_layers(
    ...
    production_area_layer.setCustomProperty(
        basemap_layer_collection.BasemapLayerCollection.COLLECTION_ID_KEY,
        basemap_layer_collection.BasemapLayerCollection.collection_id,
    )
